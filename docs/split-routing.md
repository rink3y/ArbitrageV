...
WSEI uses the native-token identity conversion. Other tokens can supply `gasConversion: { numerator, denominator, validUntil }` on their existing `TOKENS` entry. This is a conservative `numerator / denominator` in smallest token units per native wei and a `validUntil` Unix-millisecond timestamp. Missing, invalid or expired conversions make that borrow token ineligible for splits, without disabling its legacy linear search. There is no price-oracle integration or automatic market-price refresh for these manual entries. An upstream cached price source can instead supply `FindOpportunitiesRequest.splitCosts`. Never derive gas prices from token minimum-profit settings.
